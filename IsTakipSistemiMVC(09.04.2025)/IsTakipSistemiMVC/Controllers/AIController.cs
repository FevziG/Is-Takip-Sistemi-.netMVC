    ...
    public class AIController : Controller
    {
        ...
        public IActionResult HandleAIAction()
        {
            if (TempData["AIResponse"] != null)
            {
                ...
                if (command?.action == "WorkerEditPage") 
                {
                    TempData["AIResponse"] = json ;
        ...
        public IActionResult IsListeleAI()
        {
            if (TempData["AIResponse"] != null)
            {
                ...
                try
                {
                    ...
                    if (command != null && command.action == "IsListeleAI")
                    {
                        var user = _context.Personellers.FirstOrDefault(u => u.PersonelKullaniciAd == command.kullaniciAdi);
                ...
                {
                    {
                        TempData["IsListeleAIError"] = "İş listelenemedi.";
        ...
        {   
            if (TempData["AIResponse"] != null)
            {
                ...
                try 
                {
                    ...
                    if (command != null && command.action == "IsListeleTarihAI") 
                    {
                        var user = _context.Personellers.FirstOrDefault(u => u.PersonelKullaniciAd == command.kullaniciAdi);
                        Console.WriteLine("user ismi: " + user.PersonelAdSoyad);
                        var islerListesi = _context.Islers.Where(b => b.IsPersonelId == user.PersonelId && b.YapilanTarih.Value.Date == command.tarih)
                            .Join(_context.Durumlars,
                            isler => isler.IsDurumId,
                            durum => durum.DurumId,
                            (isler, durum) => new IsDetayViewModel
                ...
                {
                    TempData["IsListeleTarihAIError"] = "İş listelenemedi.";
                    Console.WriteLine("hata8" + ex.Message);
                    return View("AIPage", "Home");
                }
            }
            else 
            {
        ...
        public IActionResult WorkerEditPageAI () 
        {
            if (TempData["AIResponse"] != null)
            {
                ...
                try
                {
                    ...
                    if (command != null && command.action == "WorkerEditPage") 
                    {
                       var user = _context.Personellers.FirstOrDefault(u => u.PersonelKullaniciAd == command.kullaniciAdi);
            ...
            {
                TempData["AIWorkerEditPageError"] = "Worker Edit Page kısmına gidilemiyor.";
...
//çalışanlara fotğraf ekleme işleme yap VARBİNARY(MAX)   public byte[]? PersonelFotograf { get; set; }
//entity.Property(e => e.PersonelFotograf) .HasColumnName("personelFotograf") .HasColumnType("VARBINARY(MAX)");
