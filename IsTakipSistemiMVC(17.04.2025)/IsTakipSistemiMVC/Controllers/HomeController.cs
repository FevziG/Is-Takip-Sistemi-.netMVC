...
namespace IsTakipSistemiMVC.Controllers
{
    public class HomeController : Controller
    {
       private readonly IsTakipDbContext _context;
        ...
        public IActionResult WorkerEdit(int personelId, string adSoyad, string username, string password, int personelyetkiId, string phoneNumber, IFormFile photo)
        {
            ...
            else
            {
                ...
                {
                    photo.CopyTo(ms);
                    personel.PersonelPhoto = ms.ToArray();
                }
                }
        ...
        public IActionResult isTakip(int selectPerId) 
        {
            ...
            //----------------------------------------------------------------------------------
            TempData["secilenId"] = selectPerId;
        ...
        public IActionResult isListele() 
        {
            ...
            if (TempData["secilenId"] == null) 
            {
                return RedirectToAction("isTakipPage", "Home");
            }
            int personelId = (int)TempData["secilenId"];
            var personel = _context.Personellers.FirstOrDefault(u => u.PersonelId == personelId);
        ...
        [HttpPost]
        public IActionResult Register(string adSoyad, string username,
            string password, int birimId, string phoneNumber, IFormFile photo)
        {
            if (string.IsNullOrWhiteSpace(adSoyad) ||
               string.IsNullOrWhiteSpace(username) ||
               string.IsNullOrWhiteSpace(password) ||
            ...
            {
                photo.CopyTo(ms);
                newPersonel.PersonelPhoto = ms.ToArray();
            }
        ...
        public IActionResult AIPage()
        {
           ...
           ViewBag.AIResponse = TempData["AIResponse"];
            ...
            {
                ViewBag.AIError = TempData["AIError"];
            }
            if (TempData["AISuccess"] != null) 
            {
                ViewBag.AISuccess = TempData["AISuccess"];
            }
           return View(yetkiId);
        ...
        //public IActionResult PhotoEkle(IFormFile foto) 
        //{
        //    int? personelId = HttpContext.Session.GetInt32("personelId");
        //    var user = _context.Personellers.FirstOrDefault(u => u.PersonelId == personelId);

        //    if (foto != null && foto.Length > 0)
        //    {
        //        using (var ms = new MemoryStream())
        //        {
        //            foto.CopyTo(ms); // FOTOÐRAFI BYTE[]'E ÇEVÝR
        //            user.PersonelPhoto = ms.ToArray();
        //        }

        //        _context.SaveChanges(); // FOTOÐRAFI VERÝTABANINA KAYDET
        //    }
